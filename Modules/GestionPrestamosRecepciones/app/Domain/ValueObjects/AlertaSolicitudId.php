...
/**
 * Identificador de valor (UUID) de una {@see AlertaSolicitud}.
 * Inmutable; crear con {@see generate()} o reconstruir con {@see from()}.
 */
final readonly class AlertaSolicitudId
{
    ...
    public static function from(string $uuid): self
    {
        if (! Uuid::isValid($uuid)) {
            throw new \DomainException(
                sprintf('"%s" no es un UUID válido para AlertaSolicitudId', $uuid)
            );
        }
