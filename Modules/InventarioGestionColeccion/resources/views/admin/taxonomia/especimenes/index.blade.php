<div class="space-y-6 p-6">
    <div class="flex items-center justify-between">
        <flux:heading size="xl" level="1" class="text-blue-navy font-bold">Búsqueda de Especímenes</flux:heading>
    </div>
    ...
    <div class="rounded-lg border border-border bg-surface shadow-sm p-5 space-y-4">
        <flux:heading size="md" level="2" class="text-text-primary">Criterios de Búsqueda</flux:heading>
    ...
    @if($buscado)
        <div class="rounded-lg border border-border bg-surface shadow-sm overflow-hidden">
            <div class="px-4 py-3 bg-bg-main border-b border-border flex items-center justify-between">
                ...
                <tbody class="divide-y divide-border">
                    @forelse($especimenes as $especimen)
                        <tr class="hover:bg-bg-main transition-colors">
                            ...
                            <td class="px-4 py-3 text-text-secondary text-xs">{{ $especimen['taxonId'] }}</td>
                            ...
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center rounded px-2 py-0.5 text-xs font-semibold
                                    ...
                                    @elseif($especimen['estado'] === 'prestado') bg-warning text-white
                                    @else bg-border text-text-primary
                                    @endif">
                                    {{ ucfirst($especimen['estado']) }}
                                </span>
                            </td>
                        </tr>
                    @empty
